  ...
  return (
    <section className="lg:hidden mt-6 flex w-full border hover:shadow-lg focus:shadow-lg border-gray-600 hover:border-gray-900 focus:border-gray-900 dark:hover:border-gray-400 dark:focus:border-gray-400 md:h-96 h-64 rounded-lg relative">
      ...
      <motion.div
        initial="initial"
        animate="animate"
        className=" md:mx-3 px-6 py-3 relative  md:w-8/12 my-auto rounded-lg  md:bg-gray-100 md:dark:bg-gray-700  md:shadow-md"
      >
        <motion.div variants={stagger}>
          <a href={project.site} target="_blank" rel="noopener noreferrer">
            ...
            <div className="  sm:w-9/12  md:w-full text-sm md:text-lg ">
              <motion.p
                variants={fadeInUp}
                className="text-sm mt-2 leading-snug md:text-base"
              >
                {project.description}
              </motion.p>
              {project.techStack && (
                <motion.ul
                  variants={fadeInUp}
                  className="flex text-xs md:text-sm mt-3 font-semibold "
                >
  ...
  return (
    <section className="hidden lg:flex w-full ">
      <motion.div
        ...
        className="shadow border border-gray-400 hover:border-gray-600 dark:border-gray-600 dark:hover:border-gray-400 cursor-pointer w-4/5 ml-6 xl:mx-auto h-96 rounded-lg hover:shadow-lg focus:shadow-lg overflow-hidden relative"
      >
        <a href={project.site} target="_blank" rel="noopener noreferrer">
          <img
            className="block absolute inset-0 h-full w-full object-cover xl:object-top rounded-lg opacity-50 hover:opacity-95"
            src={project.imageLight}
            alt={project.title}
          />
        </a>
      ...
      >
        <motion.div variants={stagger}>
          ...
          >
            <motion.h1
              variants={fadeInUp}
              className=" dark:text-gray-100 mt-2 lg:mb-5 text-xl lg:text-3xl font-extrabold leading-snug tracking-tight lg:leading-14"
            >
              {project.title}
            </motion.h1>
          </a>
          <div className=" w-full  ">
            <motion.p
              variants={fadeInUp}
              className="text-right leading-snug  bg-gray-100 dark:bg-gray-700 p-4 rounded-lg"
            >
              {project.description}
            </motion.p>
            {project.techStack && (
              <motion.ul
                variants={fadeInUp}
                className="flex text-sm font-semibold mt-4 justify-end"
              >
                {project.techStack.map((s, index) => (
                  <li className="mr-3" key={index}>
                    <i>{s}</i>
          ...
          <motion.div
            variants={fadeInUp}
            className="pt-2 mt-1.5 flex justify-end"
          >
            {project.gitHub && (
              <a
                className="mr-5"
  ...
  return (
    <section className="hidden lg:flex w-full">
      <motion.div
        initial="initial"
        animate="animate"
        className="w-2/5 z-10 py-3 my-auto rounded-lg"
      >
        <motion.div variants={stagger}>
          <a href={project.site} target="_blank" rel="noopener noreferrer">
            <motion.h1
          ...
          <div className="w-full">
            <motion.p
              variants={fadeInUp}
              className="leading-snug bg-gray-100 dark:bg-gray-700 p-4 rounded-lg"
            >
              {project.description}
            </motion.p>
            {project.techStack && (
              <motion.ul
                variants={fadeInUp}
                className="flex text-sm font-semibold mt-4 justify-start"
              >
                {project.techStack.map((s, index) => (
                  <li className="mr-3" key={index}>
                    <i>{s}</i>
          ...
          <motion.div
            variants={fadeInUp}
            className="pt-2 mt-1.5 flex justify-start"
          >
            {project.gitHub && (
              <a
                className="mr-5"
      ...
      <motion.div
        ...
        className="shadow border border-gray-400 hover:border-gray-600 dark:border-gray-600 dark:hover:border-gray-400 w-4/5 -ml-12 h-96 rounded-lg hover:shadow-lg focus:shadow-lg  overflow-hidden relative"
      >
        <a href={project.site} target="_blank" rel="noopener noreferrer">
          <img
            className="block absolute inset-0 h-full w-full object-cover xl:object-top rounded-lg opacity-50 hover:opacity-95"
